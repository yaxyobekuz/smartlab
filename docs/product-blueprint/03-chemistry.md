...
## Snapshot
**Real state:** LearnStuff's most-built subject — 4 live pages under `/chemistry`: **Periodic table** (`periodic-table`, 118 elements from a 10.8 K-line `elements.js`, 43 ions, vendored balancer/molar-mass/empirical/solubility calculators + a native 105-equation worksheet generator), **Molecules** (`molecules`, 240-molecule PubChem 3D ball-and-stick viewer + VR), **Interactive lab bench** (`lab`, R3F 3D vessel — pour 14 elements + 11 reagents, 8 hardcoded reaction rules, Gemini AI-reaction for exactly 2 reagents), and **Atoms** (`atoms`, Bohr model, only 5 hand-typed atoms). Strong data, thin engines: the lab recognizes products against just 16 of the 240 molecules, and the calculators are a 13 K-line non-React `dangerouslySetInnerHTML` island.
